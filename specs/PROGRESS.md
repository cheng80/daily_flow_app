...
- V2 버전 파일 생성 (단계적 개발 방식)
  ...
  - `lib/view/main_range_view_v2.dart` 생성
    ...
    - Phase 2: 범위 선택 상태 관리 추가 완료 ✅

- Phase 0.5-1 완료: 싱글 모드 정상 동작 확인 ✅
  - 기존 `MainView`와 동일하게 동작하는지 검증 완료
  - 이벤트 표시, 스타일링 일관성 확인 완료
  - `home.dart`에 테스트 버튼 추가 완료

- Phase 0.5-2 완료: 범위 선택 기능 추가 완료 ✅
  - 범위 선택 UI 구현 (시작일/종료일 선택, 범위 표시)
  - 모드 전환 기능 (단일 ↔ 범위)
  - 범위 선택 시 일정 조회 및 표시
  - 필터링 기능 적용 (시간순/중요도순 정렬)

- Phase 0.5-3 완료: UI 개선 및 통계 연동 ✅
  - 모드 전환 시 서머리바 초기화 기능
  - 범위 모드로 전환 시 서머리바 초기화
  - 단일 모드로 전환 시 마지막 날짜의 데이터 로드 및 서머리바 계산
  - 범위 선택 시 정렬 기능 적용 (시간순/중요도순)
  - 통계 페이지에 필터 전달 기능
    - 필터가 없으면 전체 데이터로 통계 계산
    - 필터가 있으면 필터링된 데이터로 통계 계산
    - 단일 모드일 때도 날짜 범위 전달
