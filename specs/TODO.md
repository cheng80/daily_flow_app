...
**목표:** 앱 전반에서 사용되는 공용 설정 값들을 중앙에서 관리하는 클래스 제작

**필요성:**
- 현재 페이지 전환 애니메이션(`PageTransitionType`) 등 공용 설정 값이 각 사용처에 하드코딩되어 있음
- 향후 설정 변경 시 모든 사용처를 수정해야 하는 문제
- 앱 전반의 일관된 설정 관리 필요

**구현 계획:**
- [ ] `lib/app_custom/app_settings.dart` 또는 `lib/vm/app_settings.dart` 생성
  - [ ] 싱글톤 패턴 또는 정적 클래스로 구현
  - [ ] 페이지 전환 애니메이션 타입 설정 (`PageTransitionType`)
  - [ ] 기타 공용 설정 값 관리 (앱 버전, 기본값 등)
  - [ ] 설정 값 변경 시 콜백 지원 (선택)
- [ ] 모든 사용처에서 하드코딩된 설정 값 제거
  - [ ] `CustomNavigationUtil.to()` 호출 시 `transitionType` 파라미터를 설정 클래스에서 가져오도록 변경
  - [ ] 기타 공용 설정 값도 설정 클래스에서 참조하도록 변경
- [ ] 설정 값 영구 저장 지원 (선택)
  - [ ] SharedPreferences 또는 다른 저장소 연동
  - [ ] 사용자 설정 변경 시 저장 및 복원

**참고:**
- 현재 `PageTransitionType.fade`가 모든 네비게이션 호출에 하드코딩되어 있음
- 향후 다른 공용 설정 값 추가 시 확장 가능한 구조로 설계
